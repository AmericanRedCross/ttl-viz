...
Ctrl.prototype.createAsset = function(req,res) {
	...
	Asset.findOne({ 'title' :  req.body.title }, function(err, asset) {
		...
		if (asset) {
			...
			newAsset.user = req.user.username;
...
Ctrl.prototype.createUser = function(req,res) {
	 User.findOne({ 'username' :  req.body.username }, function(err, user) {
		...
		if (user) {
		    ...
		    res.redirect(localConfig.application.nginxlocation + "users");
		} else {
...
Ctrl.prototype.importCSV = function(req,res,type) {
	...
	if (file.mimetype != "text/csv") {
		...
		res.redirect(localConfig.application.nginxlocation +type+"s");
	} else {
		...
		parser.on("finish",function() {
			...
			for (var i=0;i<output.length;i++) {
				...
				if (type == "asset") {
					newEntity.user = req.user.username;
				}
				requests++;
		    	newEntity.save(function(err) {
			        ...
			        if (err) {
			        	if (err.toString().indexOf("E11000") != -1) {
							...
							if (type == "user") {
			        			err = "There is already an account associated with the username address "+val+".";
			        		}
